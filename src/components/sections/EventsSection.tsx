  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
    ...
    <div className="relative z-10 w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border border-primary/30 bg-card glow-red-strong animate-modal-in" onClick={(e) => e.stopPropagation()}>
      <div className="relative h-48 overflow-hidden rounded-t-2xl">
        ...
        <button onClick={onClose} className="absolute top-3 right-3 rounded-full bg-card/80 backdrop-blur p-2 text-foreground hover:bg-primary hover:text-primary-foreground transition-colors">
          <X className="w-4 h-4" />
        </button>
      ...
      <div className="p-5 space-y-4">
        <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
          {event.teamSize && <span className="flex items-center gap-1 rounded-full bg-secondary px-3 py-1"><Users className="w-3 h-3" /> {event.teamSize}</span>}
          {event.time && <span className="flex items-center gap-1 rounded-full bg-secondary px-3 py-1"><Clock className="w-3 h-3" /> {event.time}</span>}
          {event.coordinator && <span className="flex items-center gap-1 rounded-full bg-secondary px-3 py-1"><Phone className="w-3 h-3" /> {event.coordinator}</span>}
          {event.fee && <span className="rounded-full bg-primary/20 text-primary px-3 py-1 font-semibold">{event.fee}</span>}
        </div>
        <p className="text-sm text-muted-foreground leading-relaxed">{event.description}</p>
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-foreground mb-2 flex items-center gap-1"><Zap className="w-3 h-3 text-primary" /> Rules</h4>
          <ul className="space-y-1">{event.rules.map((r, i) => <li key={i} className="text-xs text-muted-foreground flex gap-2"><span className="text-primary font-bold">›</span> {r}</li>)}</ul>
        </div>
        <div>
